...
def data_splits_gen(input_obs, input_viterbi_labels, splits):
    ...
    for size in sizes:
        ...
        obs_idx += nobs
        yield obs, vit
    assert vit_idx == nsegments
    assert obs_idx == input_obs.shape[1]

def ecg_data_splits_gen():
    from data.ecg import obs_ecg, gt_vit_seq_ecg

    return data_splits_gen(obs_ecg, gt_vit_seq_ecg, 21)

def eeg_data_splits_gen():
    from data.eeg import obs_eeg, gt_vit_seq_eeg

    return data_splits_gen(obs_eeg, gt_vit_seq_eeg, 21)
...
if __name__ == '__main__':
    ...
    obs = np.loadtxt(args.input_obs, ndmin=2)
